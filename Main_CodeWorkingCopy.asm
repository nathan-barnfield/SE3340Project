...
wordsString:		.asciiz		"Here are the words that could have been made:\n"
.align 2
...
nl: .asciiz "\n"
space: .asciiz "   "
greeting: .asciiz "Welcome to Lexathon!\nWait while the game loads..."
...
main:
	...
	mfhi $a0
		...
		##placeholder #. Need a random number to be entered here
...
nullWordsLoop:		
			sb $zero, 0($t4)
			addi $t4, $t4, 1		#null the whole memory section
			addi $t3, $t3, 1
			bne $t3, 10999, nullWordsLoop
...
finishPrint:	la $a0, newLine
		li $v0, 4
		syscall
		jr $ra						#jump back to the caller
			
##############################################################################################################################################	
print_words:		la $t1, wordListAddr			#load the address of the array holding the found words base address
			la $t3, printString			#load the address of the string we are going to use to print the words with
			lw $t1, 0($t1)				#load the base address of the  words list from the wordListAddr array
			
			la $a0, wordsString
			li $v0, 4
			syscall
			
printLoop1:		lbu $t2, 0($t1)				#load the next character in the foudn words list
			beqz $t2, finishPrint			#if null, the list is complete and jump to the finish portion of this function
			sb $t2, 0($t3)				#store the character into the 
			addi $t1, $t1, 1
			addi $t3, $t3, 1			#increment the printString and foundWordsList addresses
...
nullPrintString1:	lbu $t5, 0($t3)
			beqz $t5, nullPrintEnd1
			sb $zero, 0($t3)			##empty the userInput String
			addi $t3, $t3, 1
			j nullPrintString1
nullPrintEnd1:		la $t3, printString
			j printLoop1				#jump back to beginning of the function so that the next word can be printed	
			
			

			
finishPrint1:	jr $ra						#jump back to the caller
exit:
